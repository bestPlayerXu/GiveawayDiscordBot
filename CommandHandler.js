...
class CommandHandler {
	...
	startGiveaway(guildId, channel, startTime, endTime, winners, prize) {
		setTimeout(() => {
			channel.send('new giveaway for ' + (endTime - startTime) / 1000 + ' seconds for `' + winners + '` lucky winners. You can get `' + prize + '`').then(m => {
        			m.react('806460058405306391');
                	        this.dbConnector.addGiveaway(guildId, channel.id, m.id, startTime, endTime, winners, prize);
                        	setTimeout(() => this.endGiveaway(m, guildId), Math.max(endTime - Date.now(), 0));
	                });
		}, Math.max(startTime - Date.now(), 0));
	}
